...
NestJS Controllers + Swagger Decorators
        в†“ (npm run swagger:generate)
OpenAPI JSON/YAML Spec (apps/store-api/swagger.json)
        в†“ (npm run generate-api)
Orval в†’ Typed React Query Hooks
        в†“
...
// DO NOT EDIT вЂ” run `npm run generate-api` to update
...
- ALWAYS use Orval-generated hooks in frontend code вЂ” NEVER write manual `fetch` or `axios` calls.
- ALWAYS run `npm run generate-api` after adding or changing backend endpoints.
- NEVER edit files in `src/shared/api/generated/` вЂ” they are auto-generated.
