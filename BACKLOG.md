...
| Admin advances order via status dropdown → payment badge flips PENDING→PAID at CONFIRMED and stays PAID through DELIVERED; PENDING→CANCELLED keeps payment PENDING; CONFIRMED(PAID)→CANCELLED keeps PAID; confirm-payment path still yields {CONFIRMED, PAID} | TASK-123-C | status↔paymentStatus coupling shipped; 380 backend tests green; needs running stack (plan 056 Scenarios A–D) |
