...
| Task ID | Description | Status | Plan |
| --- | --- | --- | --- |
| TASK-239 | `SeoSettings` admin singleton (default meta title/description, `%s \| brand` title template, default OG image, site-wide noindex toggle, editable llms.txt intro, additional `sameAs` links) — mirrors `SiteContactSettings`; wires `RevalidationNotifier` from day one. **Done:** backend module (repo/service/controllers/DTO/entity + `%s` template validation + revalidation), seed defaults, Orval regen, admin `/settings/seo` form+widget with plain-UA hints, and storefront wiring (`seo-settings-server` + root `generateMetadata`, Organization `sameAs` merge, `robots.ts` noindex, `llms.txt` intro override); typecheck/lint + all 3 builds green, store-api 947 unit + 32 int + 9 new e2e, store-admin 203 (5 new), store-client schema green; live GET/PUT smoke-tested | ✅ | 116 |
