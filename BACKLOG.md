...
| TASK-271 | [H/H] Staging deploy (after 270) — GitHub Actions: push develop (green CI) → build → GHCR → SSH staging → `compose pull && up -d` → `prisma db push` → curl health; secrets in `staging` Environment; concurrency group; result notice; runbook `docs/deploy.md` (logs/restart/rollback) in plain UA | ✅ | 124 |
