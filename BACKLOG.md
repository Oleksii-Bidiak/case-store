...
| Task ID | Description | Status | Plan |
| --- | --- | --- | --- |
| TASK-073 | **Product images (backend + frontend).** `ProductEntity` exposes no image data, so cards/PDP show placeholders and `next/image` can't be wired. Add `ProductImage` model (url, alt, sortOrder, isPrimary) + variant images, admin upload, expose on list + detail API, regen Orval. **Unblocks TASK-068-C/-F.** Highest-impact gap for an accessories store. | ✅ | docs/plans/041-product-images.md |
| TASK-073-A | Prisma migration — add `isPrimary Boolean @default(false)` to `ProductImage` + replace `@@index([productId])` with `@@index([productId, sortOrder])` | ✅ | docs/plans/041-product-images.md |
| TASK-073-B | `StorageService` abstraction — `IStorageService` interface + `LocalDiskStorageService` (local-disk impl) + `StorageModule`; add `UPLOAD_DEST` + `PUBLIC_BASE_URL` to `env.validation.ts`; gitignore `uploads/` | ✅ | docs/plans/041-product-images.md |
| TASK-073-C | `ProductImageRepository` (findByProductId, findPrimaryByProductIds, create, bulkCreate, updateMany, delete, findById) + `ProductImageEntity` gains `isPrimary` + `ProductRepository.findAll` joins primary image; `ProductEntity` gains optional `primaryImage` field | ✅ | docs/plans/041-product-images.md |
| TASK-073-D | `ProductImageService` — TDD (Red→Green): uploadImages (MIME/size guard, disk write, DB insert, cache evict), reorderImages (at-most-one-primary guard), deleteImage (cross-product guard, disk cleanup) | ✅ | docs/plans/041-product-images.md |
| TASK-073-E | `ProductImageController` (POST upload, PATCH reorder, DELETE) + multer config (memoryStorage, 5 MB limit, MIME fileFilter) + `ProductModule` wiring + `ServeStaticModule` in `AppModule` (serves `/uploads`) | ✅ | docs/plans/041-product-images.md |
| TASK-073-F | E2E tests — upload/reorder/delete auth guards (401/403), valid upload 201, oversized/non-image 400/413, reorder >1 isPrimary 400, delete 204/404; `StorageService` mocked | ✅ | docs/plans/041-product-images.md |
| TASK-073-G | Regenerate Orval API hooks for store-client + store-admin (new image endpoints + `isPrimary` on entity + optional `primaryImage` on product list response) | ✅ | docs/plans/041-product-images.md |
| TASK-073-H | Storefront `ProductCard` — render primary image (`<img>` + `onError` → `ProductThumb` fallback); badges/action slot preserved; `@next/next/no-img-element` eslint comment documents TASK-074 deferral | ✅ | docs/plans/041-product-images.md |
| TASK-073-I | Storefront `ProductImageGallery` — verification pass: typecheck clean after Orval regen; unit test for `altText()` helper (non-empty string / null / empty → fallback) | ✅ | docs/plans/041-product-images.md |
| TASK-073-J | Admin `features/product-image-manager` — upload (multi-file), image grid (thumbnail + set-primary + delete + sortOrder), drag-reorder, Sonner toasts, skeleton; FSD-compliant | ✅ | docs/plans/041-product-images.md |
| TASK-073-K | Admin `EditProductView` — wire `<ProductImageManager productId={productId} />` below `<ProductForm />` with "Product Images" heading + `<Separator />`; `CreateProductView` unchanged | ✅ | docs/plans/041-product-images.md |
| TASK-073-L | Seed data — add `ProductImage` rows for first 6 seeded products using picsum.photos URLs; idempotent upsert; `isPrimary = true` on first image per product | ✅ | docs/plans/041-product-images.md |
| TASK-073-M | Verification gate — typecheck / lint / build / unit tests / e2e all green across all 3 workspaces; zero unintended Orval file diffs; `uploads/` absent from git status | ✅ | docs/plans/041-product-images.md |
