...
| TASK-080 | **[promoted from Phase C]** Delivery + Nova Poshta — city + branch autocomplete via the NP API, persist the chosen branch in the DB, show cost + ETA in checkout. Manual delivery is the interim fallback until this ships. Sub-tasks: ⬜ **TASK-080-A** backend `DeliveryModule` — `NovaPoshtaClient` (Node.js `fetch`), `DeliveryService` (city/warehouse search + estimate, Redis cache), `DeliveryController` (`GET /api/delivery/cities`, `/warehouses`, `/estimate`), env vars `NP_API_KEY` + `NP_SENDER_CITY_REF`; TDD unit tests written first. ⬜ **TASK-080-B** `AddressDto` + `ShippingAddressData` extended with optional `npCityRef`/`npWarehouseName`/`npWarehouseRef`; `OrderService` computes `shippingCost` from NP estimate on order create (falls back to 0); `OrderRepository.createFromCart` writes cost; Orval regen. ⬜ **TASK-080-C** frontend `Combobox` primitive in `shared/ui`; `NpCityField` + `NpWarehouseField` in `features/checkout/ui` (debounced via `useDebouncedCallback`, forms.md Rule 3); replace free-text city/deliveryAddress inputs; shipping cost + ETA in `CheckoutOrderSummary`; dict strings added. ⬜ **TASK-080-D** tests — `DeliveryService` unit (mock client), delivery e2e, `Combobox` RTL, `NpCityField` RTL+MSW, updated `checkout-address-form.test.tsx`. ⬜ **TASK-080-E** admin-configurable dispatch origin — `DeliverySetting` singleton (Prisma migration: sender city ref/name + optional sender warehouse + default weight), `DeliveryRepository`, admin-only `GET`/`PUT /api/admin/delivery-settings`, store-admin settings page (NP city picker); estimate resolves origin/weight as `DeliverySetting` → `NP_SENDER_CITY_REF` → Kyiv default. Internal data, never shown to customers. | 🔄 | [docs/plans/068-nova-poshta-delivery.md](docs/plans/068-nova-poshta-delivery.md) |
