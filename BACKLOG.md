...
> New task IDs use the single monotonic counter — **next plain ID: TASK-299**.
...
| TASK-298 | `AttributeDefinitionRepository.reorder` is a naive reorder — no advisory lock, no staleness check, so two admins reordering attribute definitions at once silently interleave. It predates `common/reorder` and is now the last writer of `sortOrder` that does not use it. Next adopter of `reorderBucket` (found during TASK-295) | ⬜ | — |
...
- **New task IDs:** single monotonic counter; next plain ID **TASK-299**. Never reuse an ID.
