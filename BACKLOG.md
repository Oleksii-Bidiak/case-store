...
| Admin order list shows customer email + name (not UUID); admin detail shows Customer card with email/name; customer-facing `/api/orders` responses contain NO `customer` key (PII not leaked); status update keeps the Customer card | TASK-125 | `customer` on admin `OrderEntity` shipped; 397 store-api + 5 store-admin tests green; needs running stack (plan 058 QA A–D) |
