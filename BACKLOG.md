...
| TASK-142-D | **[EPIC / Phase 3]** Orval regen — `npm run generate:api`; verify `ProductEntity` gains `stock` + `attributes`; detail response type gains `group?`; `AddToCartDto` loses `variantId`; `npm run build` clean in both frontends. Depends: TASK-142-B + TASK-142-C. (S) | ⬜ | docs/plans/060-variant-as-product-position.md |
| TASK-142-E | **[EPIC / Phase 4a]** Storefront PDP + grid — remove `ProductVariantSelector` + `pick-default-variant`; add `ProductSiblingNavigator` (axis strips → sibling slug navigation); update `ProductDetailView` to show `product.price` / `product.stock` / `product.sku` directly; remove `variantId` from `AddToCartButton` + `MobileAtcBar`. Component tests for `ProductSiblingNavigator`. Depends: TASK-142-D. (M) | ⬜ | docs/plans/060-variant-as-product-position.md |
| TASK-142-F | **[EPIC / Phase 4b]** Admin — position form adds `stock`, `attributes` key-value editor, `groupId` select, `positionOrder`; remove inline variants section; add `/admin/product-groups` list + create/edit pages. Depends: TASK-142-D. (M) | ⬜ | docs/plans/060-variant-as-product-position.md |
| TASK-142-G | **[EPIC / Phase 5]** Seed rewrite + SEO review + final drop migration — seed creates positions + groups (absorbs TASK-128 seed work); verify sitemap lists one URL per position; verify JSON-LD per position; drop `product_variants_bak` table + `variantId` columns via final migration. Depends: TASK-142-B + TASK-142-C + TASK-142-E. (M) | ⬜ | docs/plans/060-variant-as-product-position.md |
...
| TASK-128 | Seed overhaul — enough products for pagination, an out-of-stock variant, real descriptions + characteristics, sale items; document re-seed after migrations; fix `npm run db:studio` (prisma:studio script). **NOTE: reshaped by TASK-142 — seed must create positions + groups under the new model; implementation absorbed into TASK-142-G. Do not start TASK-128 before TASK-142-A is merged.** | ⬜ | — |
