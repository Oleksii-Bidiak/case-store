...
- [ ] Exports `generateMetadata` function:
  ...
  - Calls `productControllerFindBySlug(slug)` (raw fetcher, not the hook) from
    `@/shared/api/generated/products/products`
  - On success: returns `{ title: "${product.data.name} | CaseStore", description: product.data.description ?? "View product details." }`
  - On error (any exception, including 404): returns `{ title: "Product | CaseStore" }` as a safe fallback
