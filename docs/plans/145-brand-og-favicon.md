...
- `scripts/brand-assets/og-banner.svg` — окреме hand-authored джерело 1200×630 лише для
  ...
  зліва/по центру текст «CaseStore» (великий, жирний) + один рядок теглайну
...
- `shared/config/dictionary.ts` — **одна рядкова правка** (не структурна): значення
  `meta.rootTitle` → `"CaseStore — магазин аксесуарів для телефонів"`. Жодних нових ключів
...
- [ ] `dictionary.ts`: `meta.rootTitle` = `"CaseStore — магазин аксесуарів для телефонів"`
...
| Плейсхолдер-дизайн (типографічний «M»/«CaseStore» на суцільному кольорі) виглядає непрофесійно порівняно з реальним лого                                                                                                       | Явно узгоджено умовою задачі — «власник зможе замінити файл пізніше»; шляхи файлів фіксовані й стабільні, заміна не потребує зміни коду                                                                                                                     |
