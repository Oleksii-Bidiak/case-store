...
## 14. Етап 5 — SEO follow-ups (plan 117: TASK-247)

> Автотести зелені (store-api category repo/service specs, store-client resolveSeo call-site;
> typecheck + lint чисті). Нижче — те, що видно лише на запущеному стенді (Крок 0). Схема без
> змін — колонки `Category.metaTitle`/`metaDescription` вже існують (TASK-240).
