...
- `docs/deploy/` — **Deploy & operations, in order** (UA, for non-technical operators). Numbered: the number is the step, not a chapter. Entry point `docs/deploy/00-start-here.md`. The canonical path is self-hosted VPS + Docker + Caddy; the old Vercel/Railway runbook is superseded and parked in `docs/archive/`
  ...
  - `10-capacity.md` — **which server to buy**, answered by measurement rather than inference. The k6 harness lives in `scripts/load/`. Read it before quoting any concurrency number: until TASK-392 the repo had no load test at all, and inference is what let a single shared rate-limit bucket (TASK-386) reach production unnoticed. It also documents why 429 is counted separately from other failures — a rate-limited run measures a limit, not a server
