...
export const dict = {
  ...
  footer: {
    ...
    /** TASK-483 — публічна форма «номер + телефон» у колонці «Інформація». */
    infoOrderStatus: "Статус замовлення",
    infoBlog: "Блог",
  ...
  // ── Публічна форма «Статус замовлення» (TASK-483) ─────────────────────────
  // Другий спосіб побачити замовлення — той, що не залежить від листа. Тексти
  // навмисно нічого не обіцяють про існування замовлення: сервер на будь-яку
  // невдачу відповідає однаково, тож і сторінка мусить.
  orderLookup: {
    breadcrumbHome: "Головна",
    breadcrumb: "Статус замовлення",
    heading: "Статус замовлення",
    intro:
      "Введіть номер замовлення з листа-підтвердження та номер телефону, який ви вказали при оформленні.",
    fieldNumber: "Номер замовлення",
    fieldNumberPlaceholder: "94F5F971",
    fieldNumberHint: "8 символів — можна з «#», великими чи малими літерами.",
    fieldPhone: "Телефон",
    fieldPhonePlaceholder: "+380 XX XXX XXXX",
    submit: "Перевірити",
    submitting: "Шукаємо…",
    searchAgain: "Перевірити інше замовлення",
    errors: {
      numberRequired: "Номер замовлення — це 8 символів із листа",
      phoneRequired: "Вкажіть телефон у форматі +380 XX XXX XXXX",
      // Одне повідомлення на всі невдачі — рівно тому, що сервер їх не
      // розрізняє. Сторінка, яка вгадувала б причину, підказувала б перебирачу.
      notFound:
        "Замовлення з таким номером і телефоном не знайдено. Перевірте номер у листі та телефон, який ви диктували менеджеру.",
      rateLimited:
        "Забагато спроб поспіль. Зачекайте хвилину й спробуйте ще раз.",
      generic: "Не вдалося виконати пошук. Спробуйте ще раз за хвилину.",
    },
    resultsHeading: "Знайдено замовлення",
    resultsMultiple: (count: number) =>
      `За цим номером і телефоном знайдено замовлень: ${count}`,
    orderNumber: "Номер:",
    placedOn: "Оформлено:",
    itemsHeading: "Товари",
    itemQuantity: (quantity: number) => `× ${quantity}`,
    totalsHeading: "Разом до сплати",
    ...
    // Пояснює, чому тут немає адреси й контактів — щоб це читалось як турбота
  ...
  account: {
    ...
    dashboard: {
      ...
      // --- Приєднані гостьові замовлення (TASK-485) ---------------------------
      // Показується рівно один раз — одразу після підтвердження пошти, бо саме
      // підтвердження і є моментом, коли замовлення переїжджають на акаунт.
      // Мовчки збільшити список означало б, що людина бачить у себе чужі на
      // вигляд замовлення й не розуміє, звідки вони.
      claimedOrdersHeading: "Ми знайшли ваші попередні замовлення",
      // Формулювання «: N» замість «N замовлень» — свідомо: українські числівники
      // вимагають трьох форм, а рядок із числом у кінці правильний для будь-якого
      // N і не потребує окремого помічника відмінювання заради одного банера.
      claimedOrdersBody: (count: number) =>
        `Замовлень, які ви оформлювали без входу в акаунт на цю адресу: ${count}. ` +
        "Тепер вони у вашому профілі.",
      claimedOrdersCta: "Переглянути замовлення",
      claimedOrdersDismiss: "Зрозуміло",
  ...
  // Закон дає 14 днів на повернення, а в кабінеті не було жодної кнопки: заявку
  // вміла створити лише API, і дістатися до неї покупець не міг.
  returnRequest: {
    trigger: "Подати заяву на повернення",
    triggerAria: (orderNumber: string) =>
      `Подати заяву на повернення за замовленням ${orderNumber}`,
    dialogTitle: "Заява на повернення",
    dialogDescription:
      "Оберіть, що саме повертаєте, і вкажіть причину. Ми звʼяжемося з вами, щойно розглянемо заяву.",
    itemsHeading: "Що повертаєте",
    quantityAria: (productName: string) =>
      `Кількість до повернення: ${productName}`,
    // «1 з 3» — скільки ще можна повернути з того, що куплено. Частину могли
    // повернути раніше: заявок на одне замовлення може бути кілька.
    remainingOf: (remaining: number, ordered: number) =>
      `${remaining} з ${ordered} доступно`,
    nothingLeft: "Уже повернено",
    reason: "Причина повернення",
    reasonPlaceholder: "Наприклад: не підійшов розмір",
    reasonHint: "Необовʼязково, але допоможе нам розглянути заяву швидше.",
    // Підказка стоїть поруч із вимкненою кнопкою: вимкнена кнопка без пояснення
    // читається як зламана форма, тож причину завжди видно поруч.
    nothingSelected: "Оберіть хоча б одну позицію.",
    submit: "Надіслати заяву",
    submitting: "Надсилаємо…",
    cancel: "Скасувати",
    success: "Заяву на повернення надіслано.",
    error: "Не вдалося надіслати заяву. Спробуйте ще раз.",
    loadError: "Не вдалося завантажити дані про повернення.",
