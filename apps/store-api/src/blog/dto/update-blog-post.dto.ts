...
/**
 * DTO for updating a blog post (admin-only). All fields optional — only provided
 * fields are written. Publish control (`status` + `scheduledAt`) is inherited
 * from the shared {@link PublishFieldsDto}.
 */
export class UpdateBlogPostDto extends PublishFieldsDto {
  @ApiProperty({ description: 'Post title', example: 'iPhone 16 проти iPhone 15', required: false })
  @IsOptional()
  @IsString()
  ...
  @ApiProperty({ description: 'URL-friendly slug', example: 'iphone16-vs-15', required: false })
  @IsOptional()
  @IsString()
