 ...
 *
 * The narrowing half lives in {@link ProductListWhereParams} (TASK-489) — it is
 * shared verbatim with the facet counters, so the listing and the numbers next
 * to the facet values can never describe two different slices of the catalogue.
 * What is left here is what only a PAGE needs: pagination and ordering.
 */
export interface FindAllParams extends ProductListWhereParams {
  page: number;
  limit: number;
  /**
...
export class ProductRepository {
  ...
  async findAll(params: FindAllParams): Promise<PaginatedProductsResult> {
    ...
    // The narrowing half of the query is built by the SHARED listing
    // where-builder (TASK-489) — the same one the facet counters run, so a
    // «Силікон (12)» can never lead to a page of eight. Only the on-sale ids,
    // the one filter that needs a round trip of its own, are resolved here and
    // handed to it.
    const where = buildProductListWhere(
      params,
      params.onSale ? await this.getOnSaleProductIds() : undefined,
    );
