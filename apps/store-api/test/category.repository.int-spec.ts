...
/**
 * Integration tests for the TASK-236 recursive traversal helpers
 * (`findSubtreeIds` / `findAncestorIds`) — run the REAL repository against a
 * REAL Postgres instance (no mocks). Only a live DB can prove the recursive CTE
 * is correct: that a subtree query rolls up every descendant at any depth, an
 * ancestor query walks up to the root, and — crucially — that SIBLINGS never
